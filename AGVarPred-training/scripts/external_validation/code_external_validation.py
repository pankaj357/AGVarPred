#!/usr/bin/env python3
"""
Copy of code.py adapted for external validation.
AlphaGenome path explicitly set to /data/kanaka/pankaj/alphagenome/src
"""
import os
import sys

# -------------------------------------------------------
# Add local AlphaGenome to path
# -------------------------------------------------------
sys.path.insert(0, "/data/kanaka/pankaj/alphagenome/src")
...
# -------------------------------------------------------
# PROCESS FUNCTION (SKIPPING LOGIC REMOVED)
# -------------------------------------------------------
def process_variant_file(input_file_path, dna_model, output_dir):
    ...
    try:
        ...
        # Process all variants from scratch (no skipping logic)
        all_rows = []
        all_columns = set()
        ...
        for idx, row in variants.iterrows():
            ...
            try:
                chrom = str(row["chrom"])
                # Normalize mitochondrial chromosome naming for AlphaGenome API
                if chrom == "chrMT":
                    chrom = "chrM"
                pos = int(row["pos"])
                ref = str(row["ref"])
...
def main():
    ...
    API_KEY_INDEX = int(os.environ.get("API_KEY_INDEX", RUN_ID))

    if RUN_ID >= len(API_KEYS):
        raise ValueError("RUN_ID exceeds API_KEYS")
    if API_KEY_INDEX >= len(API_KEYS):
        raise ValueError("API_KEY_INDEX exceeds API_KEYS")

    API_KEY = API_KEYS[API_KEY_INDEX]
    print(f"🚀 RUN_ID: {RUN_ID} | API_KEY_INDEX: {API_KEY_INDEX}")

    dna_model = dna_client.create(API_KEY)

    INPUT_BASE = os.environ.get("INPUT_BASE", "external_validation/processing/chunks/external_humsavar")
    OUTPUT_BASE = os.environ.get("OUTPUT_BASE", "external_validation/processing/features/humsavar")

    input_path = f"{INPUT_BASE}/run_{RUN_ID+1}"
    files = [
        f for f in Path(input_path).glob("*.txt")
        if not f.name.startswith("._")
    ]

    print(f"📂 Files: {len(files)}")

    output_dir = f"{OUTPUT_BASE}/output_test_run_{RUN_ID+1}"
    ...
    summary_dir = Path(OUTPUT_BASE) / "output"
    summary_dir.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(results).to_csv(
        summary_dir / f"summary_test_run_{RUN_ID+1}.csv", index=False
    )
