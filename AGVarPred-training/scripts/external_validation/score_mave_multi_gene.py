#!/usr/bin/env python3
"""Score MAVE Multi-Gene benchmark with correlation analysis."""
...
OUTDIR = EXT_DIR / "results/mave_multi_gene/regularized"
...
def score_mave(feature_df, pipe, labels_df, outdir):
    print("\n🔄 Scoring...")
    ...
    pred_df = pd.DataFrame({
        ...
        "prediction": preds,
        "gnomAD_AF": feature_df["gnomAD_AF"].values,
    })
    ...
    af_nonzero = pred_df.loc[pred_df["gnomAD_AF"] > 0, "gnomAD_AF"]
    characterization = {
        "benchmark": "MAVE Multi-Gene",
        "n_total": int(len(pred_df)),
        "n_pathogenic": int(y_true.sum()),
        "n_benign": int((y_true == 0).sum()),
        "n_genes": int(pred_df["gene"].nunique()),
        "AF_zero_percent": float((pred_df["gnomAD_AF"] == 0).mean() * 100),
        "median_AF": float(af_nonzero.median()) if len(af_nonzero) > 0 else 0.0,
        "mean_AF": float(pred_df["gnomAD_AF"].mean()),
        "ultra_rare_percent": float(pred_df["ultra_rare"].mean() * 100),
    }
...
def save_mave_results(pred_df, metrics, subgroup_results, characterization, outdir):
    ...
    print(f"  AF=0 %:                {characterization['AF_zero_percent']:.1f}%")
...
if __name__ == "__main__":
    print("="*70)
    print(f"MAVE MULTI-GENE BENCHMARK SCORING — Model: regularized")
    print("="*70)
    ...
    pred_df, metrics, subgroup_results, characterization = score_mave(
