#!/usr/bin/env python3
"""
Preprocess raw VEP parquets to match the format expected by the trained models.

This adds derived features (one-hot consequences, ordinal mappings) that
feature_labbling.py creates during training data preparation.
"""
...
VEP_FILES = {
    "gnomad_benign": "external_validation/gnomad_benign_vep.parquet",
    "humsavar": "external_validation/humsavar_vep.parquet",
    "mave_independent": "external_validation/mave_independent_vep.parquet",
    "vip": "external_validation/vip_vep.parquet",
    "grimm2015": "external_validation/vep_preprocessed/grimm2015_vep.parquet",
}

OUTDIR = Path("external_validation/vep_preprocessed")
OUTDIR.mkdir(exist_ok=True, parents=True)


def preprocess_vep(df):
    """Add all derived VEP features matching feature_labbling.py."""
    df = df.copy()
    if "variant_id" in df.columns:
        df = df.set_index("variant_id")
    
    # Numeric features - ensure float type
...
for name, path in VEP_FILES.items():
    ...
    out_path = OUTDIR / f"{name}_vep.parquet"
    df.to_parquet(out_path)
    print(f"  Saved: {out_path}")

print("\n✅ Done! Preprocessed VEP files saved to external_validation/vep_preprocessed/")
