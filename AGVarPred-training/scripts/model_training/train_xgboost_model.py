...
baseline_probs = dummy.predict_proba(X_test.fillna(imputer_full))[:, 1]
...
final_thresh = max(
    ...
    key=lambda t: f1_score(
        y_cal,
        (probs_cal_calibrated > t).astype(int)
    )
)
...
np.savetxt(f"{OUTDIR}/confusion_matrix.txt", confusion_matrix(y_test, preds), fmt='%d')
...
}).to_csv(f"{OUTDIR}/metrics.csv", index=False)
...
plt.tight_layout()
plt.savefig(f"{OUTDIR}/feature_importance_plot.png", dpi=200)
plt.close()
...
pd.DataFrame(shap_values, columns=X_sample.columns).to_csv(
    f"{OUTDIR}/shap_values_sample.csv", index=False
)
...
pd.DataFrame({
    ...
    f"{OUTDIR}/feature_importance_shap.csv", index=False
)
