...
def main():
    ...
    input_path = f"chunks_input_test/run_{RUN_ID+1}"
    files = list(Path(input_path).glob("*.txt"))

    print(f"📂 Files: {len(files)}")

    output_dir = f"feature_extraction_test_dataset/output_test_run_{RUN_ID+1}"
    ...
    pd.DataFrame(results).to_csv(
        f"output/summary_test_run_{RUN_ID+1}.csv", index=False
    )
